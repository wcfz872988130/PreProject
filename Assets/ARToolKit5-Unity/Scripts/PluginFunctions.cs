...
public static class PluginFunctions
{
	...
	// --begin--
	// pass
	public static bool arwOpencvTest(int param)
	{
		bool ok;
		if (Application.platform == RuntimePlatform.IPhonePlayer) 
			ok= ARNativePluginStatic.arwOpencvTest(param);
		else 
			ok=ARNativePlugin.arwOpencvTest(param);
		return ok;
	}


	public static bool arwSetInteraction(int param)
	{
		bool ok;
		if (Application.platform == RuntimePlatform.IPhonePlayer) 
			ok= ARNativePluginStatic.arwSetInteraction(param, 1);
		else 
			ok=ARNativePlugin.arwSetInteraction(param, 4);
		return ok;
	}

	public static bool arwSetInteraction2(int doInteraction, int innerSize)
	{
		bool ok;
		if (Application.platform == RuntimePlatform.IPhonePlayer) 
			ok= ARNativePluginStatic.arwSetInteraction2(doInteraction, innerSize);
		else 
			ok=ARNativePlugin.arwSetInteraction2(doInteraction, innerSize);
		return ok;
	}

	public static bool arwGetLastTrig(out float stamp)
	{
		bool ok = false;
		if (Application.platform == RuntimePlatform.IPhonePlayer)
			ok = ARNativePluginStatic.arwGetLastTrig(out stamp);
		else
			ok = ARNativePlugin.arwGetLastTrig(out stamp);
		return ok;
	}

	// pass
	public static bool arwSetSavePath(string cfg, int len)
	{
		bool ok = false;
		 
		if (Application.platform == RuntimePlatform.IPhonePlayer)
			ok=ARNativePluginStatic.arwSetSavePath (cfg, len);
		else
			ok=ARNativePlugin.arwSetSavePath (cfg, len);
		return ok;
	}

	// pass
	public static bool arwSetSavePath3()
	{
		bool ok = false;
		string path=null;
		int len=0;
		if (Application.platform == RuntimePlatform.IPhonePlayer) {
			...
			ok = ARNativePluginStatic.arwSetSavePath (path, len);
		} else {
			path = Application.dataPath + "/Resource/GameData/";
			len=path.Length;
			ok = ARNativePlugin.arwSetSavePath (path, len);
		}
		Debug.Log("arwSetSavePath3="+path);
		return ok;
	}

	// fail
/*	public static bool arwSetSavePath2(string cfg)
	{
		bool ok = false;
		if (Application.platform == RuntimePlatform.IPhonePlayer)
			ok=ARNativePluginStatic.arwSetSavePath2 (cfg);
		else
			ok=ARNativePlugin.arwSetSavePath2 (cfg);
		return ok;
	}

*/
	// pass.
	public static string arwGetWarpImagePath(int stamp, int markerID)
	{
		StringBuilder sb = new StringBuilder(256);
		bool ok=false;
		if (Application.platform == RuntimePlatform.IPhonePlayer) 
		{
			ok = ARNativePluginStatic.arwGetWarpImagePath (sb, stamp, markerID);
		}
		else 
			ok = ARNativePlugin.arwGetWarpImagePath (sb, stamp, markerID);
	...
	// pass
	public static bool arwSnapWarpImage2Disk(int markerUID, int warpScale, out int stamp)
	{
		bool ok = false;
		if (Application.platform == RuntimePlatform.IPhonePlayer)
			ok = ARNativePluginStatic.arwSnapWarpImage2Disk (markerUID, warpScale, out stamp);
		else
			ok = ARNativePlugin.arwSnapWarpImage2Disk (markerUID, warpScale, out stamp);
		return ok;
	}

	// pass
	public static string arwGetLastWarpImagePath()
	{
		StringBuilder sb = new StringBuilder(256);
		bool ok=false;
		if (Application.platform == RuntimePlatform.IPhonePlayer) 
		{
			ok = ARNativePluginStatic.arwGetLastWarpImagePath (sb);
		}
		else 
			ok = ARNativePlugin.arwGetLastWarpImagePath (sb);
		if (ok)
			return sb.ToString();
		else
			return "null";
	...
	// pass
	public static bool arwSnapWarpImage2Texture32([In, Out] Color32[] colors32, int width, int height, int markerUID)
	{
		bool ok = false;
		GCHandle handle = GCHandle.Alloc(colors32, GCHandleType.Pinned);
		IntPtr address = handle.AddrOfPinnedObject();
		if (Application.platform == RuntimePlatform.IPhonePlayer) 
			ok = ARNativePluginStatic.arwSnapWarpImage2Texture32(address, width, height, markerUID);
		else 
			ok = ARNativePlugin.arwSnapWarpImage2Texture32(address, width, height, markerUID );
		handle.Free();
		return ok;
	}


	public static bool arwGetLastWarpImageTexture32( [In, Out] Color32[] colors32, IntPtr warpImageUID)
	{
		bool ok = false;
		GCHandle handle = GCHandle.Alloc(colors32, GCHandleType.Pinned);
		IntPtr address = handle.AddrOfPinnedObject();
		if (Application.platform == RuntimePlatform.IPhonePlayer) 
			ok = ARNativePluginStatic.arwGetLastWarpImageTexture32(address, warpImageUID);
		else 
			ok = ARNativePlugin.arwGetLastWarpImageTexture32(address,warpImageUID );
		handle.Free();
		return ok;
	}
