...
public class ARController : MonoBehaviour
{
	...
	public string videoConfigurationiOS0 = "";//-preset=480p
...
//    	AR_MATRIX_CODE_GLOBAL_ID = 2830
    ...
    void Awake()
    {
		...
		Debug.Log("ARController Awake");
		#if UNITY_IOS && !UNITY_EDITOR
		ARNativePluginStatic.aruRequestCamera();
		Thread.Sleep(2000);
		#endif
		Debug.Log ("TemplateSize"+TemplateSize+",TemplateCountMax"+TemplateCountMax);
...
//		}
		...
		ARTrackedObject[] markers = FindObjectsOfType(typeof(ARTrackedObject)) as ARTrackedObject[];
		foreach (ARTrackedObject m in markers) {
			m.Load();
		}
	...
	bool UpdateAR()
	{
        ...
        if (!_sceneConfiguredForVideo) {
            ...
            if (!PluginFunctions.arwIsRunning()) {
				...
				ARTrackObjectManager.GetInstance().OnARControllerActivate();
				...
				if (_useNativeGLTexturing) {
					...
					if (_videoColor32Array0 != null) {
						...
						if (updatedTexture) {
							...
							/*
							for(int i=(int)(_videoWidth0*(_videoHeight0*(2/9.0)));i<(int)(_videoWidth0*(_videoHeight0*(2/9.0)+1));i++)
							{
								int add = _videoWidth0*(int)(_videoHeight0*(5/9.0));
								if(i%20<10)
								{
									_videoColor32Array0[i]= Color.white;
									_videoColor32Array0[i+add]= Color.white;
								}
							}
							for(int i=0;i<_videoHeight0;i++)
							{
								if(i%20<10)
								{
									_videoColor32Array0[(int)(i*_videoWidth0+_videoWidth0*(2/9.0))]= Color.white;
									_videoColor32Array0[(int)(i*_videoWidth0+_videoWidth0*(7/9.0))]= Color.white;
								}
							}
							*/
	...
	private bool ConfigureViewports()
	{
		...
		if (!haveStereoARCamera) {
			...
			_videoBackgroundCamera0.gameObject.AddComponent<ScaleMovieCamera>();
		} else {
			// Stereo display.
...
//		foreach (ARMarker m in markers) {
		ARTrackedObject[] markers = Component.FindObjectsOfType(typeof(ARTrackedObject)) as ARTrackedObject[];
		foreach (ARTrackedObject m in markers) {
            GUI.Label(new Rect(10, y, 500, 25), "Marker: " + m.UID + ", " + m.MarkerVisible);
            y += 25;
        }
    }
